...
function mostrarProyectosCronologicos() {
  ...
  ordenados.forEach((p, index) => {
    if (p.tipo === 'película') {
      html += `<div class="pelicula">🎬 ${index + 1}. ${p.titulo} (${p.anio})</div>`;
    } else {
      const temporadaText = p.temporada ? ` - T${p.temporada}` : '';
      html += `<div class="pelicula">📺 ${index + 1}. ${p.titulo}${temporadaText} (${p.anio}) - ${p.episodios} ep.</div>`;
    }
  });
...
function mostrarSoloSeries() {
  const series = proyectos.filter(p => p.tipo === 'serie').sort((a,b) => a.anio - b.anio || (a.temporada || 1) - (b.temporada || 1));
  let html = '<h2>📺 Solo Series por estreno</h2>';
  html += `<p><strong>Total de series: ${series.length}</strong></p>`;
  series.forEach((s, index) => {
    const temporadaText = s.temporada ? ` - T${s.temporada}` : '';
    html += `<div class="pelicula">${index + 1}. ${s.titulo}${temporadaText} (${s.anio}) - ${s.episodios} ep.</div>`;
  });
  mostrarEnPagina(html);
}
...
function buscarHeroe() {
  ...
  html += '<button onclick="agregarNuevoHeroe()" style="background: #4caf50; color: white; padding: 15px 30px; border: none; border-radius: 5px; cursor: pointer; font-size: 16px; margin-top: 10px;">➕ Agregar Nuevo Héroe</button>';
...
function mostrarInformacionHeroe(nombreHeroe) {
  ...
  if (heroe) {
    ...
    heroe.proyectos.forEach((proyecto, index) => {
      html += `<div class="pelicula">${index + 1}. ${proyecto}</div>`;
    });
    html += '</div>';
...
function agregarNuevoHeroe() {
  const nombre = prompt('Ingrese el nombre del nuevo superhéroe:');
  if (!nombre) return;
  
  const nuevos = prompt('Ingrese los proyectos separados por coma:');
  if (nuevos) {
    heroes.push({ 
      nombre: nombre.trim(), 
      proyectos: nuevos.split(',').map(x => x.trim()) 
    });
    mostrarEnPagina(`<h2>✅ Héroe agregado</h2>
                     <p>Se agregó <strong>${nombre.trim()}</strong> a la base de datos.</p>
...
console.log(`📺 Series: ${proyectos.filter(p => p.tipo === 'serie').length}`);
console.log(`🦸‍♂️ Total de héroes: ${heroes.length}`);
console.log('Use los botones para navegar por todo el Universo Cinematográfico de Marvel.');
